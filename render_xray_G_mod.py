...
def test(range_phi, render_radius, theta_mean, z, generator_test, N_samples, iteration, img_size):
...
def reconstruct(args, config_file):
    ...
    for iteration in range(1000):
        ...
        if iteration % args.save_every == args.save_every - 1:
            test(range_phi, render_radius, theta_mean, z, generator_test, N_samples, iteration, img_size)
...
if __name__ == "__main__":
    # Arguments
    parser = argparse.ArgumentParser(description='Finetune the latent code to reconstruct the CT given an xray projection.')
